...
simuldata_old = feval([problem, '_modelsimulate'],bigtheta,covariates); 
summsimuldata_old = feval([problem, '_summaries'],simuldata_old);
summariesmatrix(1,:) = summsimuldata_old;

covmatrix = 1./weights.^2;
% a gaussian kernel with covariance covmatrix
logkernel_old =  -log(threshold) -0.5 * ((summsimuldata_old(:)-summobs(:))'.*covmatrix.*(summsimuldata_old(:)-summobs(:))')/threshold^2;
...
simuldata = feval([problem, '_modelsimulate'],bigtheta,covariates); 
summsimuldata = feval([problem, '_summaries'],simuldata);
logkernel =  -log(threshold) -0.5 * ((summsimuldata(:)-summobs(:))'.*covmatrix.*(summsimuldata(:)-summobs(:))')/threshold^2;
